...
#' automatically writes a \pkg{OpenBUGS} script, calls the model, and saves the
#' simulations for easy access in .
#' 
...
#' \pkg{OpenBUGS} window will pop up and will freeze up. The model will now run
#' in \pkg{OpenBUGS}. It might take awhile. You will see things happening in
#' the Log window within \pkg{OpenBUGS}. When \pkg{OpenBUGS} is done, its
#' window will close and will work again.  \item If an error message appears,
#' re-run with \code{debug=TRUE}.  }
#' 
#' BUGS version support: \itemize{ \item\pkg{OpenBUGS} >=3.2.1 }
#' 
...
#' vector or list of the names of the data objects used by the model. If
...
#' \code{inits=NULL}, initial values are generated by \pkg{OpenBUGS}. If
...
#' named \file{.bug} has been eliminated because the new \pkg{OpenBUGS} feature
#' that allows the program image to be saved and later restarted uses the .bug
#' extension for the saved images.  Alternatively, \code{model.file} can be an
#' R function that contains a BUGS model that is written to a temporary model
#' file (see \code{\link{tempfile}}) using \code{\link{write.model}}.
...
#' \code{n.thin} = 1. The thinning is implemented in the OpenBUGS update phase,
#' so thinned samples are never stored, and they are not counted in
#' \code{n.burnin} or \code{n.iter}.  Setting \code{n.thin}=2, doubles the
#' number of iterations OpenBUGS performs, but does not change \code{n.iter} or
#' \code{n.burnin}.  Thinning implemented in this manner is not captured in
...
#' @param debug if \code{FALSE} (default), \pkg{OpenBUGS} is closed
#' automatically when the script has finished running, otherwise \pkg{OpenBUGS}
#' remains open for further investigation.  The debug option is not available
#' for linux execution.
#' @param DIC logical; if \code{TRUE} (default), compute deviance, pD, and DIC.
#' The results are extracted directly from the \pkg{OpenBUGS} log, which uses
...
#' returned, if \code{TRUE} file names of \pkg{OpenBUGS} output are returned
...
#' R2OpenBUGS).
#' @param OpenBUGS.pgm For Windows or WINE execution, the full path to the
#' OpenBUGS executable.  For linux execution, the full path to the OpenBUGS
#' executable or shell script (the path to the shell script is not required if
#' the OpenBUGS shell script is in the user's PATH variable).  If \code{NULL}
#' (unset) and the environment variable \code{OpenBUGS_PATH} is set the latter
#' will be used as the default.  If \code{NULL} (unset), the environment
#' variable \code{OpenBUGS_PATH} is unset and the global option
#' \code{R2OpenBUGS.pgm} is not \code{NULL} the latter will be used as the
#' default.  If none of the former are set and OS is Windows, the most recent
#' OpenBUGS version registered in the Windows registry will be used as the
#' default.  For other operating systems, the location is guessed by
#' \code{Sys.which("OpenBUGS")}.
#' @param working.directory sets working directory during execution of this
#' function; \pkg{OpenBUGS}' input and output will be stored in this directory;
#' if \code{NULL}, a temporary working directory via \code{\link{tempdir}} is
#' used.
...
#' \file{coda[1:nchains].txt} should be removed after \pkg{OpenBUGS} has
...
#' @param bugs.seed Random seed for \pkg{OpenBUGS}.  Must be an integer between
#' 1-14.  Seed specification changed between WinBUGS and OpenBUGS; see the
#' OpenBUGS documentation for details.
...
#' available from OpenBUGS.
#' @return If \code{codaPkg=TRUE} the returned values are the names of coda
#' output files written by \pkg{OpenBUGS} containing the Markov Chain Monte
...
#' points if you wish to run \pkg{OpenBUGS} for further iterations}
